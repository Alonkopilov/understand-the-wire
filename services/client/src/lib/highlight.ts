  ...
  while (cursor < text.length) {
    ...
    for (const rule of rules) {
      ...
      if (match !== null && match.index === cursor && match[0].length > 0) {
        flush()
        if (rule.expand) tokens.push(...expandInterpolation(match[0], rule.kind))
...
const BASH_RULES: Rule[] = [
  ...
  {
    kind: 'keyword',
    pattern: /\b(?:if|then|else|elif|fi|for|in|do|done|while|until|case|esac|function|return|export|local|set|source)\b/y,
  },
  { kind: 'func', pattern: /\b(?:aws|kubectl|flux|curl|openssl|tee|mkdir|chmod|sed|ln|bash|echo|sleep|cat)\b/y },
  { kind: 'number', pattern: /\b\d+(?:\.\d+)?\b/y },
  { kind: 'punct', pattern: /[|&;()<>]+/y },
]
