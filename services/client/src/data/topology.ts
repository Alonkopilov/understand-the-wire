...
export const LAYERS: Layer[] = [
  ...
  { id: 'network', label: 'AWS network', caption: 'VPC 10.50.0.0/16 · eu-central-1' },
...
export const NODES: MapNode[] = [
  ...
  },
  {
    ...
    why: [],
    sources: [
      { path: 'infrastructure/modules/cloudflare/main.tf', label: 'cloudflare/main.tf' },
  ...
  /* --------------------------------------------------------------- network */
  {
    ...
    why: [],
    sources: [
      { path: 'infrastructure/modules/network/vpc.tf', label: 'network/vpc.tf' },
      { path: 'infrastructure/modules/network/variables.tf', label: 'network/variables.tf' },
    ],
    live: ({ aws }) => {
      if (!aws?.vpc) return []
      return [
  ...
  },
  {
    ...
    why: [],
    sources: [
      { path: 'infrastructure/modules/network/subnets.tf', label: 'network/subnets.tf' },
      { path: 'infrastructure/modules/network/route-tables.tf', label: 'network/route-tables.tf' },
    ],
    live: ({ aws }) => {
      if (!aws) return []
      return aws.subnets
        .filter((subnet) => subnet.public)
        .map((subnet): Fact => ({ label: subnet.zone, value: `${subnet.cidr} · ${subnet.id}`, mono: true }))
    },
  },
  {
    id: 'igw',
    ...
    row: 1,
    why: [],
    sources: [{ path: 'infrastructure/modules/network/internet-gateway.tf', label: 'network/internet-gateway.tf' }],
  },
  {
    id: 'alb',
    ...
    why: [],
    sources: [
      { path: 'infrastructure/modules/load-balancer/main.tf', label: 'load-balancer/main.tf' },
      { path: 'infrastructure/modules/load-balancer/listeners.tf', label: 'load-balancer/listeners.tf' },
    ],
    status: ({ aws }) => {
      if (!aws?.loadBalancer) return 'unknown'
      const targets = aws.loadBalancer.targets
      if (targets.length === 0) return 'warn'
      return targets.every((target) => target.health === 'healthy') ? 'ok' : 'warn'
    },
    live: ({ aws }) => {
  ...
  },
  {
    ...
    row: 2,
    why: [],
    sources: [{ path: 'infrastructure/modules/load-balancer/security-groups.tf', label: 'load-balancer/security-groups.tf' }],
  },
  {
    id: 'target-group',
    label: 'Target group',
    ...
    why: [],
    sources: [
      { path: 'infrastructure/modules/load-balancer/target-groups.tf', label: 'load-balancer/target-groups.tf' },
  ...
  },
  {
    ...
    row: 3,
    why: [],
    sources: [{ path: 'infrastructure/modules/network/nat.tf', label: 'network/nat.tf' }],
  },
  {
    id: 'private-subnets',
    ...
    row: 3,
    why: [],
    sources: [{ path: 'infrastructure/modules/network/subnets.tf', label: 'network/subnets.tf' }],
    live: ({ aws }) => {
      if (!aws) return []
      return aws.subnets
        .filter((subnet) => !subnet.public)
  ...
  },
  {
    ...
    why: [],
    sources: [
      { path: 'infrastructure/modules/network/endpoints.tf', label: 'network/endpoints.tf' },
      { path: 'infrastructure/modules/network/security-groups.tf', label: 'network/security-groups.tf' },
    ],
  },
  {
    id: 'node-sg',
    ...
    row: 4,
    why: [],
    sources: [{ path: 'infrastructure/modules/control-plane-node/security-groups.tf', label: 'control-plane-node/security-groups.tf' }],
  },
  ...
  /* --------------------------------------------------------------- compute */
  {
    ...
    why: [],
    sources: [
      { path: 'infrastructure/modules/control-plane-node/main.tf', label: 'control-plane-node/main.tf' },
      { path: 'infrastructure/modules/control-plane-node/iam.tf', label: 'control-plane-node/iam.tf' },
    ],
    live: ({ cluster }) => {
      ...
      return [
        ...
        { label: 'Ready', value: node.ready ? 'true' : 'false', status: node.ready ? 'ok' : 'error' },
  ...
  },
  {
    ...
    why: [],
    sources: [
      { path: 'infrastructure/modules/control-plane-node/init-control-plane.sh.tpl', label: 'init-control-plane.sh.tpl' },
    ],
  },
  {
    id: 'traefik',
    ...
    row: 0,
    why: [],
    sources: [{ path: 'k8s/apps/frontend/ingress.yaml', label: 'apps/frontend/ingress.yaml' }],
  },
  ...
  /* --------------------------------------------------------------- cluster */
  {
    ...
    row: 0,
    why: [],
    sources: [{ path: 'k8s/clusters/prod/flux-system/gotk-sync.yaml', label: 'flux-system/gotk-sync.yaml' }],
  },
  {
    id: 'flux',
    label: 'Flux',
    ...
    why: [],
    sources: [
      { path: 'k8s/clusters/prod/infrastructure-configs.yaml', label: 'clusters/prod/infrastructure-configs.yaml' },
      { path: 'k8s/clusters/prod/vars.yaml', label: 'clusters/prod/vars.yaml' },
    ],
    status: ({ flux }) => {
      if (!flux || flux.resources.length === 0) return 'unknown'
      return flux.resources.every((resource) => resource.ready) ? 'ok' : 'error'
    },
  },
  {
    id: 'controllers',
    ...
    why: [],
    sources: [
      { path: 'k8s/infrastructure/controllers/helm-external-secrets.yaml', label: 'helm-external-secrets.yaml' },
      { path: 'k8s/infrastructure/controllers/helm-prometheus-community.yaml', label: 'helm-prometheus-community.yaml' },
    ],
  },
  {
    id: 'configs',
    ...
    why: [],
    sources: [
      { path: 'k8s/infrastructure/configs/cluster-secret-store.yaml', label: 'cluster-secret-store.yaml' },
      { path: 'k8s/infrastructure/configs/external-secrets-grafana.yaml', label: 'external-secrets-grafana.yaml' },
    ],
  },
  {
    id: 'apps',
    ...
    why: [],
    sources: [
      { path: 'k8s/apps/frontend/deployment.yaml', label: 'apps/frontend/deployment.yaml' },
  ...
  /* -------------------------------------------------------------- identity */
  {
    ...
    why: [],
    sources: [
      { path: 'infrastructure/modules/oidc-bucket/main.tf', label: 'oidc-bucket/main.tf' },
      { path: 'infrastructure/modules/control-plane-node/init-control-plane.sh.tpl', label: 'init-control-plane.sh.tpl' },
    ],
  },
  {
    ...
    sources: [
      { path: 'infrastructure/modules/oidc-bucket/main.tf', label: 'oidc-bucket/main.tf' },
      { path: 'infrastructure/modules/service-account/main.tf', label: 'service-account/main.tf' },
    ],
  },
  {
    id: 'irsa',
  ...
  },
  {
    ...
    why: [],
    sources: [
      { path: 'infrastructure/modules/parameter-store/main.tf', label: 'parameter-store/main.tf' },
      { path: 'infrastructure/envs/prod/main.tf', label: 'envs/prod/main.tf' },
    ],
  },
  {
    ...
    row: 1,
    why: [],
    sources: [{ path: 'infrastructure/modules/control-plane-node/iam.tf', label: 'control-plane-node/iam.tf' }],
  },
]
